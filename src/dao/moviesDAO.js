...
export default class MoviesDAO {
  ...
  static async getMoviesByCountry(countries) {
    ...
    */
    ...
    let cursor
    try {
      // TODO Ticket: Projection Find movies matching the "countries" list, but only
      // return the title and _id. Do not put a limit in your own implementation, the
      // limit here is only included to avoid sending 46000 documents down the wire.
      cursor = await movies
        .find({
          countries: {
            $in: countries
  ...
  static castSearchQuery(cast) {
    const searchCast = Array.isArray(cast) ?
      cast :
      Array(cast)
  ...
  static genreSearchQuery(genre) {
    ...
    */
    ...
    // TODO Ticket: Text and Subfield Search Construct a query that will search for
    // the chosen genre.
    const query = {
      genres: {
        $in: searchGenre
  ...
  static async facetedSearch({
    ...
    */
    ...
    const queryPipeline = [
      matchStage, sortStage, skipStage, limitStage, facetStage
      // TODO Ticket: Faceted Search Add the stages to queryPipeline in the correct
      // order.
    ]
  ...
  static async getMovies({
    ...
    */
    ...
    // TODO Ticket: Paging Use the cursor to only return the movies that belong on
    // the current page
    ...
    try {
      ...
      const totalNumMovies = page === 0 ?
        await movies.countDocuments(query) :
        0
  ...
  static async getMovieByID(id) {
    try {
      ...
      */
      ...
      // TODO Ticket: Get Comments Implement the required pipeline.
      const pipeline = [{
          $match: {
            _id: ObjectId(id)
          }
